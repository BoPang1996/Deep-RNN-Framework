...
class HMDB(data.Dataset):
    def __init__(self, video_path, slice_num, batch_size, frame_num=9, action_num=51, img_size=368, overlap_rate=0.25):
        self.samples = []
        actions = sorted(os.listdir(video_path+'/' + 'train'))
        ...
        for b, video_name in enumerate(video_names):
            ...
            for batch in range(self.slice_num):
                for fn in range(0, length):
                    action[batch, fn] = self.action_emb[video_name.split('/')[1]]
