...
def main(prefix, image_size, K, T, gpu, p):
	...
	with tf.device("/gpu:%d" % gpu[0]):
		model = MCNET(image_size=[image_size, image_size], batch_size=1, K=K,
		              T=T, c_dim=c_dim, checkpoint_dir=checkpoint_dir, is_train=False)
	...
	with tf.Session(config=config) as sess:
		...
		for i in range(len(testfiles)):
			...
			for j in range(int(tokens[1]), int(tokens[2]) - K - T - 1, n_skip):
				...
				seq_batch = np.zeros((1, image_size, image_size, K + T, c_dim), dtype="float32")
				diff_batch = np.zeros((1, image_size, image_size, K - 1, 1), dtype="float32")
				for t in range(K + T):
				...
				xt = seq_batch[:, :, :, K - 1]
				pred_data[0] = sess.run(model.G, feed_dict={model.diff_in: diff_batch, model.xt: xt})
				...
				for t in range(K + T):
					...
					cssim[t] = ssim.compute_ssim(Image.fromarray(cv2.cvtColor(target, cv2.COLOR_GRAY2BGR)),
					                             Image.fromarray(cv2.cvtColor(pred, cv2.COLOR_GRAY2BGR)))
