...
class MCNET(object):
	def __init__(self, image_size, batch_size=32, c_dim=3,
		...
		self.K = K
		self.T = T
		self.diff_shape = [batch_size, self.image_size[0], self.image_size[1], K - 1, 1]
		self.xt_shape = [batch_size, self.image_size[0], self.image_size[1], c_dim]
		self.target_shape = [batch_size, self.image_size[0], self.image_size[1], K + T, c_dim]
	...
	def load(self, sess, checkpoint_dir, model_name=None):
		...
		if ckpt and ckpt.model_checkpoint_path:
			...
			_saver.restore(sess, os.path.join(checkpoint_dir, model_name))
