...
class PolygonNet(nn.Module):
	...
	def forward(self, input_data1, first, second, third):
		...
		padding_f = Variable(torch.zeros([bs, 1, 1, 28, 28])).cuda()
	...
	def test(self, input_data1, len_s):
		...
		result = Variable(torch.zeros([bs, len_s])).cuda()
		...
		padding_f = Variable(torch.zeros([bs, 1, 1, 28, 28])).cuda()
		input_s = Variable(torch.zeros([bs, 1, 1, 28, 28])).cuda()
		input_t = Variable(torch.zeros([bs, 1, 1, 28, 28])).cuda()
		...
		second = Variable(torch.zeros([bs, 1, 28 * 28 + 3])).cuda()
		second[:, 0, 28 * 28 + 1] = 1
		third = Variable(torch.zeros([bs, 1, 28 * 28 + 3])).cuda()
		third[:, 0, 28 * 28 + 2] = 1
		output = torch.cat((output, second, third), dim=2)
