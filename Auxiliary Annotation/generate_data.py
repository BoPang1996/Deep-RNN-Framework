...
files = glob(root_path + 'img/' + dataset + '/*/*.png')

count = 0

for file in tqdm(files):
	json_file = root_path + 'label' + file[len(root_path) + 3:-15] + 'gtFine_polygons.json'
	...
	for obj in objects:
		if obj['label'] in selected_classes:
			...
			I_obj_new.save(root_path + 'new_img/' + dataset + '/' + str(count) + '.png', 'PNG')
			...
			with open(root_path + 'new_label/' + dataset + '/' + str(count) + '.json', 'w') as jsonfile:
